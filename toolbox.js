...
function Toolbox(){
	...
	$(".sidebar").on("click", ".sideBarItem", function(){
        //updates anything on the canvas into the pixel array
	...
	var addToolIcon = function(icon, name, popupName){
		var itemHTML = "<div title='"+ popupName +"' class='sideBarItem' id='"+ name + "sideBarItem'> <img class='sideBarImg' src='" + icon +"'></div>"; 
		$(".sidebar").append(itemHTML); // title attribute added to the div so that tippy.js can use it to display the popover
        TippyPopup() //the tippy
	};
	...
	this.addTool = function(tool){
		...
		addToolIcon(tool.icon, tool.name,tool.popupName);
	...
	this.selectTool = function(toolName){
		...
		for(var i = 0; i < this.tools.length; i++){
			if(this.tools[i].name == toolName){
				...
				$("#" + toolName + "sideBarItem").css("border", "2px solid red");
